...
const createWebhook = async (data, id) => {
    await pool.query(`INSERT INTO webhooks (id, name, description, listID, sheetID, tabName, dateCreated, timeCreated, listname) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);`,
...
const deleteWebhooks = async (ids) => {
    ...
    ids.forEach(async (id) => {
        ...
        const { rows } = await pool.query('DELETE FROM webhooks WHERE id=$1', [id]);
        if (rows.length - res.rows.length !== 1) issues.push(1);
    });
    return (issues.length === 0) ? true : false;
}

const checkWebhookExists = async (listID) => {
    const { rows } = await pool.query('SELECT * FROM webhooks WHERE listid=$1', [listID]);
    if (rows.length === 1) return true;
}

const retrieveSingleWebhook = async (listID) => {
    const { rows } = await pool.query('SELECT sheetid, tabname FROM webhooks WHERE listid=$1', [listID]);
    return rows[0];
}
