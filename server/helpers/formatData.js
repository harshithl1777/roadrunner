...
const formatDesc = (data) => {
    return data.map(card => {
        ...
        desc.forEach((item1) => {
            ...
            if (itemCopy.includes('package_name') || itemCopy.includes('packagename')) card.pkgName = item1.slice(item1.search(':')+2);
            ...
            else if (itemCopy.includes('targetgeos')) card.geo = item1.slice(item1.search(':')+2);
